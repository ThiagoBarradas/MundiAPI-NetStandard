...
;
        ...
        internal void ValidateResponse(HttpResponse _response, HttpContext _context)
        {
            if (_response.StatusCode == 400)
                throw new ResponseException(@"Invalid request", _context);

            if (_response.StatusCode == 401)
                throw new ResponseException(@"Invalid API key", _context);

            if (_response.StatusCode == 403)
                throw new ResponseException(@"Business validation error", _context);

            if (_response.StatusCode == 404)
                throw new ResponseException(@"An informed resource was not found", _context);

            if (_response.StatusCode == 500)
                throw new ResponseException(@"Internal server error", _context);
